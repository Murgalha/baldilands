...
public static class InventoryController {
    ...
    private static bool IsPart(string part) {
        if (part.Equals("head") || part.Equals("torso") || part.Equals("hand") ||
    ...
    private static void Unequip(Hero H) {
        ...
        while (true) {
            ...
            Console.WriteLine("> Head ({0})", H.Equip.Head.HasValue
                                                  ? H.Equip.Head.Value.Name.Capitalize()
                                                  : "Empty");
            Console.WriteLine("> Torso ({0})", H.Equip.Torso.HasValue
                                                   ? H.Equip.Torso.Value.Name.Capitalize()
                                                   : "Empty");
            Console.WriteLine("> Hand ({0})", H.Equip.Hand.HasValue
                                                  ? H.Equip.Hand.Value.Name.Capitalize()
                                                  : "Empty");
            Console.WriteLine("> Leg ({0})",
                              H.Equip.Leg.HasValue ? H.Equip.Leg.Value.Name.Capitalize() : "Empty");
            Console.WriteLine("> Weapon ({0})", H.GetWeapon().HasValue
                                                    ? H.GetWeapon().Value.Name.Capitalize()
                                                    : "Empty");
