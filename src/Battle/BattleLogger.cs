...
public class BattleLogger {
	...
	private void _Save(ICreature c, bool IsPlayer, string cmd, int damage, bool crit, bool EnemyDodge) {
		string WeaponName = (c.Equip.Weapon.HasValue ? c.Equip.Weapon.Value.Name : "powerful strike");
		if(IsPlayer) {
			if(damage == 0) {
				if(cmd.Equals("ranged attack") && ((!c.Equip.Weapon.HasValue) ||
												   !c.Equip.Weapon.Value.Type.Equals("ranged")))
				...
				if(cmd.Equals("attack")) {
					...
					this._TurnLog += "You shoot the enemy with your " + c.Equip.Weapon.Value.Name + ". It loses " + damage + " life point" + (damage > 1 ? "s" : "");
				}
				if(crit)
			...
			if(damage == 0) {
				...
				if(cmd.Equals("attack")) {
					...
					this._TurnLog += "The enemy shoots you with its " + c.Equip.Weapon.Value.Name + ". You lose " + damage + " life point" + (damage > 1 ? "s" : "");
				}
				if(crit)
					this._TurnLog += ". It is a critical hit!";
			}
	...
	public void SaveTurnLog(ICreature c1, bool IsPlayer1, string cmd1, int damage1, bool crit1, bool dodge1) {
